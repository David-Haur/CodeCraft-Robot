...
class Map():
    def __init__(self):
        ...
        self.mapinfo_new = [[''] * 100 for _ in range(100)]
...
def read_map():  # 初始化地图信息
    ...
    while True:
        ...
        if line == "OK":
            ...
            return map_info
        for char in line:
            if char in ['1', '2', '3', '4', '5', '6', '7', '8', '9']:  # 工作台
        ...
        if map_info.robots[i].on_task_dest == -1:  # 如果该机器人没有任务：
            if not map_info.robots[i].goods_type:  # 且机器人没有物品携带
                ...
                if len(map_info.wt_to_be_purchased_4567):  # 同时，如果有加工工作台已经生产出产品了，说明得给机器人一个任务，让它走到对应工作台的地方
                    ...
                    map_info.robots[i].route = find_route_to_dest(wt.id, i)   # 让机器人找到去对应工作台的路线
                    ...
                    wt.on_task_robot = i
                    map_info.robots[i].route = find_route_to_dest(wt.id, i)  # 让机器人找到去对应工作台的路线
                # 如果没有任何一个工作台有产品出来，不给任何机器人分配任务——这是最开始的情况，那么机器人没有路线
                # else:
                #     logging.info("机器人" + str(i) + "此时没有路线")
...
def reset(robot_id):
    ...
    if map_info.robots[robot_id].task == 1:
        ...
        map_info.robots[robot_id].pointx = 0  # pointx设置为0，开始寻找下一阶段的路
        map_info.robots[robot_id].route = find_route_to_dest(map_info.robots[robot_id].on_task_dest, robot_id)
        return
    ...
    map_info.robots[robot_id].pointx = 0    # pointx设置为0，开始寻找下一阶段的路
    map_info.robots[robot_id].route = []
...
def check(robot_id):
    ...
    if map_info.robots[robot_id].on_task_dest > -1:  # 0~8
        # logging.info("机器人的目标工作台编号是：" + str(map_info.robots[robot_id].on_task_dest))
        # logging.info("机器人的工作是：" + str(map_info.robots[robot_id].task))
        # 如果当前机器人的任务是买入
        if map_info.robots[robot_id].task == 1:  # buy
            ...
            if map_info.robots[robot_id].goods_type == map_info.worktables_list[map_info.robots[robot_id].on_task_dest].type:
                # logging.info("匹配！当前携带物品为：" + str(map_info.robots[robot_id].goods_type))
...
# 碰撞检测——该机器人对特定机器人（由Robot_id选择）的碰撞检测：
def collision_detect(robot_id, delta_radian, line_speed):
    for i in range(4):  # 遍历四个机器人
        if i != robot_id:   # 如果当前机器人i和指定的机器人Robot_id不是同一个
            d, _ = dist(map_info.robots[robot_id].loc, map_info.robots[i].loc)  # 求一下二者距离
            if d < 2.12 and \
...
class AStar:
    ...
    def IsValidPoint(self, x, y):
        ...
        if x <= 0 or y <= 0 or x >= 100 or y >= 100:
            return False
        # 点在障碍物内
        if map_info.mapinfo[100 - y][x - 1] == '#':   # 数组的0和坐标0需要进行换算，输入的xy是坐标
            # logging.info(str(100-y) + "," + str(x-1) +"  point is invalid，点在障碍物内")
            return False
        return True
    ...
    def ProcessPoint(self, x, y, parent, robot_num, worktable_num):
        if not self.IsValidPoint(x, y):
            # logging.info("this point is invalid")
            return
        p = Point(0, 0) # 先建立一个point对象，用于存储这个点的字符坐标信息
        p.x = x
        p.y = y
        if self.IsInCloseList(p):   # 这个点已经遍历了，返回
            # logging.info("this point is passed")
            return
        if not self.IsInOpenList(p):
            p.parent = parent  # 此时，point新增了parent属性
            p.cost = self.TotalCost(p, robot_num, worktable_num)
            self.open_set.append(p)
            # logging.info("this point is" + str(self.open_set[-1].x) + ", " + str(self.open_set[-1].y) + \
            #              "parent is" + str(p.parent.x) + ", " + str(p.parent.y))
    ...
    def Astar_for_route(self, robot_num: int, worktable_num: int) -> list:
        start_point = Point(map_info.robots[robot_num].loc[0], map_info.robots[robot_num].loc[1])  # 机器人实体所在位置（起始点）
        # logging.info("real loc=" + str(map_info.robots[robot_num].loc) + "  start loc=" + str(start_point.x) + ", " + str(start_point.y))
        start_point.cost = 0
...
# 找到去工作台的路（我们得把字符坐标变成实际坐标）
def find_route_to_dest(worktable_num, robot_num):
    astar = AStar()
    # dest = map_info.worktables_list[worktable_num].loc  # 目的工作台的实际坐标
    # logging.info("目的工作台坐标为：" + str(dest))
    raw_route = astar.Astar_for_route(robot_num, worktable_num)  # 原始字符路径：从终点到起点
    # logging.info("raw route = " + str(raw_route))

    # 对路径进行优化：
    # optimal_route = []
    # for index, item in enumerate(raw_route):
    #     if index == 0:
    #         optimal_route.append(item)
    #         continue
    #     如果当前点与前一个点刚好处于45°位置上：
    #     if abs(item[0] - optimal_route[index-1][0]) == 1 and abs(item[1] - optimal_route[index-1][1]) == 1:
    #         x = item[0]
    #         y = item[1]
    #         if (map_info.mapinfo[y-1-100][x-1] == '#'):  # 下方有障碍物：
    #             point = [optimal_route[index-1][0]+1][y]
    #             optimal_route.append(point)
    #             optimal_route.append(item)
    #         elif (map_info.mapinfo[y-1-100][x-1] == '#')
    # 把字符坐标变成实际坐标
    for index, loc in enumerate(raw_route[:-1]): # index是每项的下标、loc取出的是每项的坐标
        x = loc[0]
        y = loc[1]
        left_x = x - 1
        right_x = x + 1
        ...
        down_y = y - 1
        # 这里是某个坐标点上下左右点的地图信息：
        left_point_info = map_info.mapinfo[100 - y][left_x - 1]
        right_point_info = map_info.mapinfo[100 - y][right_x - 1]
        up_point_info = map_info.mapinfo[100 - up_y][x - 1]
        down_point_info = map_info.mapinfo[100 - down_y][x - 1]

        # if map_info.mapinfo[100 - y][x - 1] == '#' 判断是否是墙的语句
        # raw_route[index][0] = raw_route[index][0] / 2 - 0.25
        # raw_route[index][1] = raw_route[index][1] / 2 - 0.25
        is_changed = 0
        # 未携带物品时的路径优化
        if map_info.robots[robot_num].goods_type == 0:
            if left_point_info != '#' and right_point_info != '#' and up_point_info != '#' and down_point_info != '#':
                raw_route[index][0] = raw_route[index][0] / 2 - 0.25
                raw_route[index][1] = raw_route[index][1] / 2 - 0.25
            else:
                if left_point_info == '#':  # 左边有墙，说明x轴只减0.05即可
                    raw_route[index][0] = raw_route[index][0] / 2 - 0.05
                    raw_route[index][1] = raw_route[index][1] / 2 - 0.25
                    is_changed = 1  # 说明y轴已经减过了，之后不能再减了
                if right_point_info == '#': # 右边有墙，减去0.45
                    raw_route[index][0] = raw_route[index][0] / 2 - 0.45
                    raw_route[index][1] = raw_route[index][1] / 2 - 0.25
                    is_changed = 1
                if up_point_info == '#':
                    if is_changed == 1: # 如果我左边已经有墙了，现在又需要对y轴处理：
                        raw_route[index][1] = (raw_route[index][1] + 0.25) * 2    # 先变回来
                    else:
                        raw_route[index][0] = raw_route[index][0] / 2 - 0.25
                    raw_route[index][1] = raw_route[index][1] / 2 - 0.45    # 统一处理y轴变化
                if down_point_info == '#':
                    if is_changed == 1: # 如果我右边已经有墙了，现在又需要对y轴处理：
                        raw_route[index][1] = (raw_route[index][1] + 0.25) * 2    # 先变回来
                    else:
                        raw_route[index][0] = raw_route[index][0] / 2 - 0.25
                    raw_route[index][1] = raw_route[index][1] / 2 - 0.05
        else:
            # 对携带物品时的处理：
            if left_point_info != '#' and right_point_info != '#' and up_point_info != '#' and down_point_info != '#':
                raw_route[index][0] = raw_route[index][0] / 2 - 0.25
                ...
                if left_point_info == '#':  # 左边有墙，说明x轴要变化：-0.25 + 0.28 = 0.03
                    raw_route[index][0] = raw_route[index][0] / 2 + 0.03
                    raw_route[index][1] = raw_route[index][1] / 2 - 0.25
                    is_changed = 1  # 说明y轴已经减过了，之后不能再减了
                if right_point_info == '#': # 右边有墙，减去0.53
                    raw_route[index][0] = raw_route[index][0] / 2 - 0.53
                    raw_route[index][1] = raw_route[index][1] / 2 - 0.25
                    is_changed = 1
                if up_point_info == '#':
                    if is_changed == 1: # 如果我左边已经有墙了，现在又需要对y轴处理：
                        ...
                        raw_route[index][0] = raw_route[index][0] / 2 - 0.25
                    raw_route[index][1] = raw_route[index][1] / 2 - 0.53    # 统一处理y轴变化
                if down_point_info == '#':
                    if is_changed == 1: # 如果我右边已经有墙了，现在又需要对y轴处理：
                        raw_route[index][1] = (raw_route[index][1] + 0.25) * 2    # 先变回来
                    else:
                        raw_route[index][0] = raw_route[index][0] / 2 - 0.25
                    raw_route[index][1] = raw_route[index][1] / 2 + 0.03

    # 对于最后一个点的处理：
    x = raw_route[-1][0] / 2 - 0.25
    y = raw_route[-1][1] / 2 - 0.25
    raw_route[-1][0] = x
    raw_route[-1][1] = y
    # logging.info("actual route = " + str(raw_route))
    return raw_route


# 让指定机器人走去指定的坐标，返回到指定坐标的距离
def action2(robot_num, dest: tuple) -> float:
    # logging.info("正在往" + str(dest) + "移动")
    ...
    # 开始往前走
        ...
        if d > 0.1:  # 以速度为2，那么必须减速的距离得大于0.09m（肯定有点误差，之后调参就行）
            map_info.robots[robot_num].forward(1)
        else:
...
def walk_through_path():
    for robot_num in range(4):
        # 拿到机器人的路线信息（这个是由manage方法得出的）
        route = map_info.robots[robot_num].route

        # 增加路径为空的判断，直接退出该方法，机器人什么都不做
        if not route:
            return

        # 增加分数大于200000分的判断，如果超了，直接啥都不干啦，哈哈哈哈哈哈哈
        # if map_info.money > 200000:
        #     break

        num = map_info.robots[robot_num].pointx
        pointTo = route[num]  # 机器人要走第几个点的坐标信息
        # logging.info("机器人的pointx = " + str(num) + "   机器人要去：" + str(pointTo))
        # 如果离下个点距离小于0.1，且不是最后一个点，说明该去下一个点了
        if action2(robot_num, pointTo) < 0.3 and num < len(route) - 1:
            map_info.robots[robot_num].pointx += 1

        # 如果指向最后一个点了，即走到目的地了，则把速度变为0.5，渐进靠近目标，并开始买卖：
        if num == len(route) - 1:
            map_info.robots[robot_num].forward(0.7)
            # map_info.robots[robot_num].rotate(0.2)
            x0 = map_info.robots[robot_num].loc[0]
            y0 = map_info.robots[robot_num].loc[1]
            dis = np.sqrt((x0-pointTo[0])**2 + (y0-pointTo[1])**2)
            # logging.info(str(robot_num) + "号距离目标：" + str(dis))
            # 只有距离小于0.4，才能购买、卖出
            if dis < 0.4:
                if map_info.robots[robot_num].check_flag:   # 如果检查标志被置位，说明要check一下，然后看下一个机器人
                    # logging.info("重新计算路线")
                    if check(robot_num):
                        # logging.info("新路线是：" + str(map_info.robots[robot_num].route))
                        continue
                if map_info.robots[robot_num].task == 1:    # 任务是买，则买东西，并置位check_flag，下一帧要不能继续买了，直接走check。
                    # logging.info("开始买！！！")
                    map_info.robots[robot_num].buy()
                    map_info.robots[robot_num].check_flag = True
                elif map_info.robots[robot_num].task == 2:  # 任务是卖，则卖掉东西
                    map_info.robots[robot_num].sell()
                    map_info.robots[robot_num].check_flag = True
...
if __name__ == '__main__':
    ...
    finish()
    # logging.info("finish read map")
    # logging.info(str(map_info.mapinfo))
    while True:
        line = sys.stdin.readline()  # 读入每一帧的信息
        # logging.info("finish read line")
        if not line:
            break
        ...
        sys.stdout.write('%d\n' % frame_id)  # 写当前帧
        task_manager()
        # logging.info("task_manager is finished")
        walk_through_path()
        # logging.info("========================")
        finish()
